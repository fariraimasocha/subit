...
import { clampOverlay, overlayFilter, type Overlay } from './overlays.ts'
...
test('captions alone keep the subtitles filter on the source video', () => {
  const { filter, inputs } = overlayFilter([], 'cues.ass', 1080)
  assert.deepEqual(inputs, [])
  assert.equal(filter, '[0:v]subtitles=cues.ass:fontsdir=fonts[v]')
})
...
test('images composite in order and the captions land last', () => {
  ...
  assert.equal(
    ...
    '[1:v]scale=432:-1[s1];' +
      ...
      '[o2]subtitles=cues.ass:fontsdir=fonts[v]',
  )
})
