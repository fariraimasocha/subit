...
export type CutoutCardImageProps = ComponentProps<"img"> & {
  /** Kept from the upstream next/image API: stretch to fill the media box. */
  fill?: boolean
}

/**
 * Upstream renders next/image. This app is TanStack Start on Vite with no image
 * optimiser, so it is a plain <img>: `fill` becomes absolute inset-0 and the
 * `sizes` prop goes with it. Everything else is the registry component.
 *
 * Uses `fill` by default; parent `CutoutCardMedia` should be `relative` with a
 * defined block size.
 */
